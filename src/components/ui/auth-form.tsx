...
function Auth({ className, onSuccess, ...props }: AuthProps) {
...
function AuthSignIn({ onForgotPassword, onSignUp, onSuccess }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    try {
      ...
      setFormState((prev) => ({ ...prev, error: getAuthErrorMessage(error.code || 'default') }));
    } finally {
...
function AuthResetSuccess({ onSignIn }: AuthResetSuccessProps) {
  return (
    ...
    >
      ...
      <h1 className="mb-2 text-3xl font-semibold text-foreground">Check your email</h1>
